## THIS CODE IS FOR FINDING PATHWAYS ACTIVITY BASED CORRELATION FOR PATIENTS DE GENES IN PATHWAY GENES
## STAGE 4 
## CODE 1
# Created By: Banabithi Bose
# Date Created: 4/28/2020

##### We will combine all the pathways for a pair and take union of DE genes for correlation.

############## FOR EXPRESSION samples################
...
colnames(PC_Correlation)<-c("patient","cellLine","reactome_pathway","description","eucledian","spearman","pearson","cca","pat_de","cell_de","union_de","reactome_genes","de_in_reactome")

processCorr<- function(cell_reactome_files,pat_reactome,pat_de_genes,SampleName){
  
  tryCatch( 
    {
      load(paste0("/users/home/bbose/PATIENT_CELL_PIPELINE3/OVARIAN/Sample_expression/",SampleName,".Rda", sep = ""))
      ...
      Ly<-data.frame(ccle_expr[union_de_genes,cellLine_id,drop=F])## cell line's expression with union DE genes
      ...
      if (nrow(reactomeLx)==0||nrow(reactomeLx)==1){
        ...
        reactome_expr_sim_scores<-cbind(paste0(str_sub(SampleName,1,-5)),cellLine_id,reactome_pathNames,reactome_descriptions,reactome_euc_Score,reactome_spearScore,reactome_pearScore,reactome_ccaScore,length(patients_DE),length(cellLines_DE),
        ...
        colnames(reactome_expr_sim_scores)<-c("patient","cellLine","reactome_pathway","description","eucledian","spearman","pearson","cca","pat_de","cell_de","union_de","reactome_genes","de_in_reactome")
          ...
          if(abs(mean(x$V1,na.rm=TRUE))=="NaN"){
            ...
            tryCatch(
              ...
              reactome_ccaScore<-abs(correlcca$cor)
                ...
                reactome_ccaScore<-"NA"
                #cat(paste0(SampleName,cellLine_id,intersect_reactome," 2 "),file="/users/home/bbose/PATIENT_CELL_PIPELINE3/ERROR_FILES/reactome_Cell_CellTypeCorrExpr.txt",sep="\n",append=TRUE)
        ...
        reactome_expr_sim_scores<-cbind(SampleName,cellLine_id,reactome_pathNames,reactome_descriptions,reactome_euc_Score,reactome_spearScore,reactome_pearScore,reactome_ccaScore,length(patients_DE),length(cellLines_DE),
                                    length(unique(union_de_genes)),length(unique(reactome_pathway_genes$V1)),length(unique(Common_DE_genes_pathway)))
        reactome_expr_sim_scores<-data.table(reactome_expr_sim_scores)
        colnames(reactome_expr_sim_scores)<-c("patient","cellLine","reactome_pathway","description","eucledian","spearman","pearson","cca","pat_de","cell_de","union_de","reactome_genes","de_in_reactome")
        #return(reactome_expr_sim_scores)
    ...
    error = function(error_condition) {
      #cat(paste0(SampleName,cellLine_id,union_reactome," : ",error_condition),file="/users/home/bbose/PATIENT_CELL_PIPELINE3/ERROR_FILES/Sim1.txt",sep="\n",append=TRUE)
      cat(paste0(error_condition),file="/users/home/bbose/PATIENT_CELL_PIPELINE3/ERROR_FILES/Sim1.txt",sep="\n",append=TRUE)
...
processExprFiles <- function(fileNames,processCorr){
  
  #SampleName <- str_sub(fileNames[1],end=-5)
  SampleName <- str_sub(fileNames,end=-5)
  print(SampleName)
  load(paste0("~/PATIENT_CELL_PIPELINE3/OVARIAN/Patients_Pathway/Pat_reactome_expr/",SampleName,"_pat_de_gene.Rda"))# patient's pathways
  load(paste0("~/PATIENT_CELL_PIPELINE3/OVARIAN/Patient_DE_genes/DE_Gene/",SampleName,"_pat_de_genes.Rda")) # patient's DM genes
  
  cell_reactome_files<-list.files("/users/home/bbose/PATIENT_CELL_PIPELINE3/CCLE_Pathway/Cell_reactome/", pattern="*.Rda")#1018 cell lines with reactome pathways
  cell_reactome_files<-cell_reactome_files
  Corr_2<-lapply(cell_reactome_files,processCorr,pat_reactome,pat_de_genes,SampleName)
  PC_Correlation1<-rbindlist(Corr_2)
  PC_Correlation<-rbind(PC_Correlation,PC_Correlation1)[-1,]
  #save(PC_Correlation,file=paste0("/users/home/bbose/PATIENT_CELL_PIPELINE3/SIMILARITIES/reactome_Expr_Sim/",SampleName,".Rda"))
...
fileNames1<-gsub("(_pat_de_gene)","",unique(list.files("/users/home/bbose/PATIENT_CELL_PIPELINE3/OVARIAN/Patients_Pathway/Pat_reactome_expr/", pattern="*.Rda")))#1061
fileNames2<-unique(list.files("/users/home/bbose/PATIENT_CELL_PIPELINE3/OVARIAN/Sample_expression/", pattern="*.Rda"))#1091
fileNames<-intersect(fileNames1,fileNames2)[151:368]#368, patients have sample expression as well as reactome pathways


Patient_Cell_Reactome_Expr_Sim_2<-rbindlist(parLapplyLB(cl,fileNames,processExprFiles,processCorr))
save(Patient_Cell_Reactome_Expr_Sim_2,file="/users/home/bbose/PATIENT_CELL_PIPELINE3/OVARIAN/SIMILARITIES1/Patient_Cell_Reactome_Expr_Sim_2.Rda")
stopCluster(cl)
