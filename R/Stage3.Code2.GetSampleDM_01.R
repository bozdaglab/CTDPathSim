...
GetSampleDMFun<-function(All_methylation,parallel=c('TRUE','FALSE'),ncores=2, CTDDirectory="~"){
  if((is(All_methylation, "SummarizedExpirament"))){
    All_methylation<-assay(All_methylation)
  }
  
  
  dir.create(paste0(CTDDirectory,"/Patient_DM_genes/"))
  dir.create(paste0(CTDDirectory,"/Patient_DM_genes/UP_Gene/"))
  dir.create(paste0(CTDDirectory,"/Patient_DM_genes/DOWN_Gene/"))
  dir.create(paste0(CTDDirectory,"/Patient_DM_genes/DE_Gene/"))
    processFiles <- function(fileNames){
        ...
        save(upgene,file=paste0(CTDDirectory,"/Patient_DM_genes/UP_Gene/",SampleName,"_upgene.Rda"))
        save(downgene,file=paste0(CTDDirectory,"/Patient_DM_genes/DOWN_Gene/",SampleName,"_downgene.Rda"))
        save(pat_dm_genes,file=paste0(CTDDirectory,"/Patient_DM_genes/DE_Gene/",SampleName,"_pat_dm_genes.Rda"))
