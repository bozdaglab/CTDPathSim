...
run_edec_stage_1 <- function(meth_bulk_samples,
    ...
    estimated_proportions <- gtools::rdirichlet(num_samples,1 * rep(1/num_cell_types, num_cell_types))

    estimated_cell_type_meth <- estimate_meth_qp(meth_bulk_over_inf_loci,estimated_proportions)

    # ---------------------------------------------------------------------------
    # Compute residual sum of squares for initial estimates
    # ---------------------------------------------------------------------------

    rss <- norm(meth_bulk_over_inf_loci - estimated_cell_type_meth %*% t(estimated_proportions),"F")^2
    ...
    while (it < max_its & rss_diff > rss_diff_stop) {
        ...
        estimated_proportions <- estimate_props_qp(meth_bulk_over_inf_loci,estimated_cell_type_meth)
        estimated_cell_type_meth <- estimate_meth_qp(meth_bulk_over_inf_loci,estimated_proportions)
        new_rss <- norm(meth_bulk_over_inf_loci - estimated_cell_type_meth %*% t(estimated_proportions),"F")^2
    ...
    aic <- num_samples * log(rss/num_samples) + 2 * num_estimated_variables +
